...
last_object_x_pos_offset=0  # 上一次的X轴偏移量，用于计算微分项
#-----------------------------------------------------------------------
try:
    while True:
        # --- 1. 只有不在调整时，才进行采样和推理 ---
        if not is_adjusting:
            ...
            for result in results:
                ...
                for box in boxes:
                    ...
                    if abs(object_x_pos_offset) > 100:  # 物体偏移大于阈值时，才进行舵机调整

                        # 比例项 (P)
                        proportional_term = KP * object_x_pos_offset * DEG_PER_PIX
                        
                        # 微分项 (D) 用于减少震荡并增加阻尼
                        derivative_term = KD * (object_x_pos_offset - last_object_x_pos_offset) * DEG_PER_PIX

                        # 计算舵机调整的角度
                        servo_pan_current_angle = secure_servo_pan_angle(servo_pan_current_angle - proportional_term - derivative_term)
                        ...
                        # 更新最后的偏移量，用于计算微分项
                        last_object_x_pos_offset = object_x_pos_offset

                        # 设置调整标志
                        is_adjusting = True
                        last_adjust_time = time()

                        break  # 这一轮已经触发动作了，跳出循环
